const validateProps = (props: any, componentDoc: any, componentProps: any) => {
  const augment: any = {};
  const msg: string[] = [];
  const result = Object.keys(componentProps).every((k) => {
    const hasProp = Object.prototype.hasOwnProperty.call(props, k);
    ...
    if (componentProps[k].isRequired && !hasProp) {
      if (Object.prototype.hasOwnProperty.call(componentProps[k], 'default')) {
